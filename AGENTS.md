...
- **Bundled `security` plugin guards are classified on a two-axis policy.** Every guard in `src/bundled-plugins/security/policies/` exports a `GUARD_*_SEVERITY: SecuritySeverity` constant (`'low' | 'medium' | 'high'`) and `tool.before` accepts EITHER the tier permission (`security.bypass.low` / `.medium` / `.high`) OR the per-guard permission (e.g. `security.bypass.gitExfil`) — OR-check, both axes work forever. The two axes are:
  - **high — audience-leak.** Bypass sends data to a third-party audience outside the operator's control loop. Channel readers, push-notification previews, remote git hosts (GitHub/GitLab/anywhere). Inhabitants: `outboundSecret`, `systemPromptLeak`, `gitExfil`, `gitRemoteTainted`. **No role auto-bypasses high.** Per-call ack required from every role, including `owner`. The canonical motivating case is **owner-in-public-channel**: even an owner-permissioned operator asking the agent to "post deploy status to #general" must not silently include a stack-trace `Bearer ghp_…` line. The high-tier bypass for one role is opt-in via explicit operator grant of the per-guard string in `roles.<role>.permissions[]`.
...
**Layer 4.5 wraps the `agent-browser` binary to work around upstream issue [vercel-labs/agent-browser#1083](https://github.com/vercel-labs/agent-browser/issues/1083) ("headed silently ignored on existing session").** Empirically, when a daemon is already running with a headless browser and a subsequent command requests headed mode (`--headed` or `AGENT_BROWSER_HEADED=1`), the existing headless browser is reused regardless — the `navigate` path in `cli/src/commands.rs` strips the `headless` field, and the auto-launch path in `execute_command` doesn't consult the cached `launch_hash` when the existing browser is still alive. Three upstream fix PRs ([#660](https://github.com/vercel-labs/agent-browser/pull/660), [#370](https://github.com/vercel-labs/agent-browser/pull/370), [#387](https://github.com/vercel-labs/agent-browser/pull/387)) have been open and unmerged for months as of agent-browser 0.27.0, so we patch this locally by mv-aliasing the real binary to `/usr/local/bin/agent-browser.real` and dropping a POSIX shell wrapper at the original path. The wrapper invokes `agent-browser close` (idempotent — emits "No active sessions" and exits 0 when no daemon is running) immediately before any browser-launching command when headed mode is requested, then exec-passes through to `.real`. This forces a clean relaunch instead of reusing the stale headless browser.

**Trigger semantics match the upstream flag parser.** `AGENT_BROWSER_HEADED=1|true|TRUE|True` and any argv form of `--headed` / `--headed=true` / `--headed=1` activate the pre-close. Falsy env values (`0`, `false`, empty, anything else) skip it. The wrapper consults both the env var and argv every invocation, so callers that flip `AGENT_BROWSER_HEADED` per-command — common in agent-driven loops — get the pre-close at the right boundary.

**Subcommand denylist short-circuits to avoid recursion and unnecessary work.** The wrapper allow-passes `close|quit|exit|status|session|skills|auth|install|upgrade|doctor|dashboard|confirm|deny` straight through to `.real` without running the pre-close. `close|quit|exit` are the close path itself (without the denylist, the wrapper would issue a pre-close before every user-issued `close`, doubling the work and visibly logging the close path twice). `status|session|skills|auth|...` don't touch the browser daemon at all. `install|upgrade|doctor` run setup/diagnostic flows that don't need a relaunch. `dashboard` is a separate daemon per the upstream source code. Layer 5's `agent-browser install --with-deps` (which calls the wrapper at build time, because Layer 4.5 runs before Layer 5) passes through the denylist unchanged — verified by the `Chrome-for-Testing download still uses the shimmed agent-browser binary` test.

**Re-entrancy is defended at two levels.** First, the `close|quit|exit` denylist prevents the pre-close from ever calling itself: the pre-close inside the wrapper invokes `"$real" close`, which is the real binary directly, not the wrapper. Second, `_TYPECLAW_AGENT_BROWSER_HEADED_HANDLED=1` is set in the env for both the pre-close call AND the final exec; the wrapper checks this guard at the very top of the script and bypasses straight to `.real` when set. This second guard exists for future-proofing: if a future agent-browser subcommand we don't recognize spawns a child `agent-browser` process while `AGENT_BROWSER_HEADED=1` is still in the env, the child sees the guard and skips the recursive pre-close.
