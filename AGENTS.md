...
| --------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `owner`   | `[{ kind: 'tui' }]`            | `channel.respond`, `cron.schedule`, `cron.modify`, `security.bypass.low`, `security.bypass.medium`, `security.bypass.high`, plus the union of all plugin-registered `security.bypass.*` (the bundled security plugin sets `ownerWildcardExclusions: []` under the role-tower model — see § Permissions § role tower) |
| `trusted` | `[]`                           | `channel.respond`, `cron.schedule`, `security.bypass.low`, `security.bypass.medium`                                                                                                                                                                                                                                  |
| `member`  | `[]`                           | `channel.respond`, `security.bypass.low`, `subagent.spawn`, `subagent.cancel`, `subagent.output`                                                                                                                                                                                                                     |
| `guest`   | `[]`                           | (none)                                                                                                                                                                                                                                                                                                               |
...
- **Bundled `security` plugin guards are classified on a two-axis policy.** Every guard in `src/bundled-plugins/security/policies/` exports a `GUARD_*_SEVERITY: SecuritySeverity` constant (`'low' | 'medium' | 'high'`) and `tool.before` accepts EITHER the tier permission (`security.bypass.low` / `.medium` / `.high`) OR the per-guard permission (e.g. `security.bypass.gitExfil`) — OR-check, both axes work forever. The two axes are:
  - **high — direct audience-leak.** Bypass sends data to a third-party audience outside the operator's control loop with NO operator-visible intermediate step. Channel readers, push-notification previews, remote git hosts that the operator did NOT configure (i.e. retargeted mid-session). Inhabitants: `outboundSecret`, `systemPromptLeak`, `gitRemoteTainted`. **`owner` bypasses; `trusted`, `member`, `guest` do not.** The canonical motivating case is **owner-in-public-channel**: an owner-permissioned operator asking the agent to "post deploy status to #general" can silently leak a stack-trace `Bearer ghp_…` line. Under the role-tower model, that defense lives in `roles.owner.match[]` discipline — the default is TUI-only, where a human is present. Configs that widen `roles.owner.match[]` to a channel author should either narrow the match or strip `security.bypass.high` (and the wildcard sentinel) from `roles.owner.permissions[]` for those origins.
  - **medium — silent-attack OR operator-reviewable state.** Two sub-shapes share this tier because they have the same defense story (operator review catches it before it escapes). (a) _silent-attack_: bypass produces attacker-favorable state in model context without immediate operator visibility. `bash env` dumps every credential; `read .env` returns the file; `curl http://169.254.169.254/...` returns IAM creds; `session_search` returns secret-shaped hits. (b) _operator-reviewable state_: bypass writes to a file the operator force-commits and reviews before the privileged effect takes hold (`typeclaw.json#roles` is restart-required so the operator has wall-clock time; `cron.json` jobs fire on schedule, again wall-clock-decoupled from the write; `git push` to a remote the operator already configured is going to a known audience even if attacker-injected prose is in the commits). Inhabitants: `secretExfilBash`, `secretExfilRead`, `ssrf`, `sessionSearchSecrets`, `gitExfil`, `rolePromotion`, `cronPromotion`. **`owner` and `trusted` bypass; `member`, `guest` do not.**
  - **low — noisy, immediately recoverable.** Bypass produces an operator-visible side effect the operator can undo. No inhabitants today. The tier exists so member's `bypass.low` grant has a forward-compat home — when a future guard ships at low, every role auto-bypasses it without a role-config edit. **`owner`, `trusted`, `member` all bypass; `guest` does not.**

  Built-in role wiring forms a strict tower: `owner` carries `bypass.low + bypass.medium + bypass.high` AND the wildcard sentinel (the sentinel expands to every plugin-contributed `security.bypass.*` because the bundled security plugin sets `ownerWildcardExclusions: []`). `trusted` carries `bypass.low + bypass.medium`. `member` carries `bypass.low`. `guest` carries no bypasses. Operators who want to _narrow_ a role's tier cap (e.g. remove `bypass.high` from owner for a channel-matched owner role, OR remove `bypass.medium` from trusted to restore pre-rebalance ergonomics) declare the explicit permission list in `roles.<role>.permissions[]` — that replaces the full default. Operators who want to _widen_ a lower role (e.g. give member `bypass.gitExfil` for a specific guard) add the per-guard string via the OR-check, e.g. `roles.member.permissions: [..., "security.bypass.gitExfil"]`.

  **`gitRemoteTainted` is classified `high`** (the breach pattern PR #134 was written to defend against: re-point origin to attacker-controlled remote, then push). Under the audience-leak axis the audience is a remote git host the operator did NOT configure. Owner-only bypass by default. The two-step defense fires regardless of how `gitExfil` is granted: the recorder (`recordGitRemoteTaintIfAny`) runs for any actor who could execute the `set-url` (per-guard `bypassGitExfil` OR the medium-tier permission, which now includes trusted by default), so the second-step taint check has state to consult on the eventual push.

  **`gitExfil` is classified `medium`** (reclassified from `high`). A `git push` to a CLEAN, operator-configured remote is not direct audience-leak — the audience is inside the operator's perimeter. The retarget-and-push attack stays blocked by `gitRemoteTainted` (still high, owner-only). Trusted bypasses by default, which means trusted users can push to operator-configured remotes without per-call acks but cannot retarget the remote first. The recorder-vs-checker split is what makes this safe — see the recorder-vs-checker rule of thumb below.

  **`rolePromotion` is classified `medium`** (reclassified from `high`). The breach pattern is the same as before — a `member`-role speaker asks the agent to "promote me", the agent edits `roles.<role>.permissions[]` or `match[]`, the schema is valid, `managedConfig` accepts it, `nonWorkspaceWrite` allowlists `typeclaw.json` — but the privilege escalation does NOT take effect until reload/restart, and `roles` is `restart-required` in `FIELD_EFFECTS`. The operator sees the file change in `git log`, in `typeclaw reload` output, and in auto-backup commits BEFORE the new mapping takes hold. The intermediate operator-visible step is the medium-tier signature. Owner and trusted bypass by default; member and guest are blocked. The guard's diff and false-positive shapes are unchanged — only the tier moved. The companion `grantRole()` in `src/permissions/grant.ts` still writes `roles.<name>.match[]` directly via `writeFileSync` and bypasses `tool.before` by construction; its only production caller is the role-claim flow (`src/role-claim/controller.ts`), gated by an operator-issued pairing code from the host CLI.

  **`cronPromotion` is classified `medium`** (reclassified from `high`). Same shape as `rolePromotion`, applied to the deferred-execution surface. `cron.json` is force-committed by the auto-backup plugin, the change appears in `git log` and backup commits, and the cron consumer dispatches by schedule — there is wall-clock time between the privileged write and the privileged execution during which the operator can revert or disable. The two-step shape ("do a privileged write now, run the privileged thing later") is unchanged; what changed is the tier-default for trusted, which now bypasses without ack. Operators who want to restore the pre-rebalance behavior for trusted explicitly remove `security.bypass.medium` from `roles.trusted.permissions[]` (declaring the full list manually since user-declared `permissions[]` replaces the built-in default). The guard's diff and false-positive shapes are unchanged — only the tier moved.

  **Tradeoff documented**: the reclassifications widen trusted's default authority over the agent's own state in exchange for fewer interactive acks on legitimate operator-delegated work. The defense for trusted now leans on operator review of auto-backup commits to `typeclaw.json` and `cron.json`. Deployments that don't review backup commits should keep trusted's match list narrow OR explicitly subtract `security.bypass.medium` from trusted's default tier.
...
- **The recorder-vs-checker split inside the security plugin is load-bearing.** `recordGitRemoteTaintIfAny` runs unconditionally (gated only by "would the command actually run", i.e. ack or any actor that can bypass `gitExfil` — via the per-guard string or via the medium-tier permission), independent of `checkGitRemoteTaintedGuard`'s block decision. The split is what makes the `gitExfil → medium` tier reclassification safe for trusted: trusted now bypasses `gitExfil` via `bypass.medium`, so trusted's first-step `git remote set-url` succeeds AND the recorder fires; on the second-step push, trusted lacks `bypass.high` and `bypassGitRemoteTainted`, so the high-tier `gitRemoteTainted` check fires on the taint that was just recorded. The same shape applies to explicit operator-granted per-guard `bypassGitExfil` for any other role: `gitRemoteTainted` is its own per-guard permission AND its own high-tier guard, distinct from `gitExfil`. Collapsing recorder and checker back into a single function silently disables the two-step taint defense for any actor holding `bypassGitExfil` OR `bypass.medium`.
...
**Persistent-`$HOME` overlay (`link_persistent_home_files`).** The container's `$HOME` (default `/root`) lives on Docker's writable overlay and is wiped on every `stop`+`start` cycle, but some tools store credentials at `~/.<tool>/<file>` and expect them to survive restarts. The entrypoint shim emits a `link_persistent_home_files` helper that symlinks the few `$HOME`-side files we want to persist into `/agent/.typeclaw/home/`, which lives in the bind-mounted agent folder on the host. Today's only resident is `~/.codex/auth.json` (Codex CLI OAuth credential; codex rewrites it in place when it rotates tokens). The persistent root is gitignored by `buildGitignore()`'s template (`.typeclaw/home/`) so credentials never enter version control.

The helper is called on both network-policy paths immediately before `start_xvfb` (off-path: right before `exec bun run typeclaw`; on-path: after iptables rules install, after `setpriv` capability-drop window closes, before `exec setpriv … -- bun run typeclaw`). Symlinking is unconditional and idempotent — `ln -sfn` creates a dangling link on first boot (codex's first login writes through it to the persistent location) and atomically replaces an existing link on subsequent boots. The `-n` flag is load-bearing: without it, a previous container life that wrote a real `~/.codex/` directory before this code shipped would cause `ln -sf` to dereference into it and create `auth.json` inside the old dir instead of replacing the dir with a symlink.

The helper symlinks the **file**, not the directory. Codex writes other state to `~/.codex/` over time (`history.jsonl`, `log/`, `config.toml`); leaving those ephemeral keeps the persistence scope tight to credentials and lets each new container start with fresh CLI state. Future credentials get added as file-by-file additions inside `link_persistent_home_files` (e.g. `~/.gemini/oauth_creds.json`) rather than by widening to directory-level links. The naming convention is `/agent/.typeclaw/home/<relative-path-under-HOME>` so the persistent tree mirrors `$HOME` shape one-for-one.

The persistent root is overridable via `TYPECLAW_PERSIST_HOME_ROOT` — but only the shim's executable tests set it. Production never does, so the in-container path is always `/agent/.typeclaw/home/`. The override exists exclusively so behavioral tests can verify symlink semantics against a real tmpdir on the test host without touching `/agent` (which doesn't exist on developer machines or CI runners). It is NOT a public configuration knob; do not document it as one.

**Why a symlink, not a bind mount of `~/.codex/` itself.** Mounting a host directory at `/root/.codex/` would require user action (a new entry in `typeclaw.json#mounts`), tie credential persistence to a per-agent config rather than a typeclaw-shipped behavior, and add a second class of mount the user has to reason about. The symlink approach makes persistence a system-shipped invariant — it works the same on every typeclaw agent without configuration. The cost is one extra `ln -sfn` per container boot, which is unmeasurable.

**Why the file lives inside the agent folder, not in `~/.typeclaw/` on the host.** The agent folder is already the bind-mounted boundary — credentials, `.env`, and managed config all live there. Putting persistent-`$HOME` files in a sibling location (e.g. `~/.typeclaw/agents/<container>/home/`) would split the agent's state across two host directories, complicating backup, migration, and the cloning model where `tar c agent-folder/ | ssh remote tar x` is supposed to be a complete migration. Co-locating with the agent folder keeps the "what is an agent's state?" question answerable by "everything in that one directory."
...
- **Off-path** (`TYPECLAW_NETWORK_BLOCK_INTERNAL≠1`): `link_persistent_home_files` is called first (seeds the credential symlink before the agent reads `~/.codex/auth.json`), then `start_xvfb`, then `exec bun run typeclaw "$@"`. setpriv is a no-op on this path (no caps were granted), so its only effect on Xvfb is to ensure the helper's body is identical between paths.
- **On-path** (`TYPECLAW_NETWORK_BLOCK_INTERNAL=1`): iptables OUTPUT rules are installed first (this requires NET_ADMIN, which PID 1 still has), then `link_persistent_home_files` seeds the symlink, then `start_xvfb` spawns Xvfb under setpriv with NET_ADMIN dropped, then the final `exec setpriv … -- bun run typeclaw "$@"` drops NET_ADMIN from the agent too. Net effect: only the shim's iptables-installation phase ever holds NET_ADMIN; neither Xvfb nor the agent inherits it. `link_persistent_home_files` runs without NET_ADMIN (it doesn't need it — only `mkdir` and `ln`) but is placed AFTER iptables so a failure in the helper cannot prevent the egress lockdown from taking effect.
