...
- **The per-agent Dockerfile FROMs the prebuilt `typeclaw-base` image at the CLI's own version, and that is load-bearing.** A user who runs `npm install typeclaw@X.Y.Z` and then `typeclaw start` gets a per-agent Dockerfile whose first line is `FROM ghcr.io/typeclaw/typeclaw-base:X.Y.Z`. The version equality is enforced by `release.yml`: the same CI job that publishes `typeclaw@X.Y.Z` to npm also pushes `ghcr.io/typeclaw/typeclaw-base:X.Y.Z` (image first, then npm — see the ordering rationale in `.github/workflows/release.yml`). The CLI never picks a different tag and never falls back to `:latest`; "the version is the pin" is a structural invariant, not a heuristic. The version is resolved at module-load time by `src/init/cli-version.ts`, which reads its own `package.json` (relative to the loaded source tree) and decides whether to emit the pinned form. **Consequence: removing or republishing a `typeclaw-base:X.Y.Z` tag after release breaks every installed copy of `typeclaw@X.Y.Z` on the next `typeclaw start --build`. GHCR has no immutability flag, so this is a process rule, not a technical one. Don't.**
- **Dev mode falls back to inlining the heavy stack.** When the agent folder's `package.json` declares `typeclaw` via a `file:` or `link:` spec (typeclaw symlinked from a local checkout), `resolveBaseImageVersion` returns `null` and `buildDockerfile` emits today's old form (`FROM oven/bun:1-slim` + every heavy layer). This is the chicken-and-egg fix: the version in the dev tree is the next-to-be-released one and the matching `:X.Y.Z` tag does not exist on GHCR yet, so a dev `typeclaw start` would `docker pull` 404 if we tried to pin. The same fallback fires when the agent folder has no `package.json` at all (tests, fresh tmp dirs, scaffolding-in-progress) — pinning the versioned base image only when we have positive evidence of a real install. The classification rule is in `isInstalledAgent` in `src/init/cli-version.ts`; the layer split between versioned and inline lives in `renderVersionedDockerfile` / `renderInlineDockerfile` inside `src/init/dockerfile.ts`. **Consequence: when editing `src/init/dockerfile.ts` to ship a new heavy-stack layer, you must update BOTH `buildBaseDockerfile` (so the next release's base image carries it) AND `renderInlineDockerfile` (so dev runs and tests see it without going through GHCR). The drift-guard describe block in `dockerfile.test.ts` catches the most common slippage between the two but is not exhaustive — run `typeclaw start --build` in a dev agent folder before pushing.**
...
**Bumping the pinned version.** Edit the four constants in `src/init/dockerfile.ts` (`CURL_IMPERSONATE_VERSION`, `CURL_IMPERSONATE_SHA256_AMD64`, `CURL_IMPERSONATE_SHA256_ARM64`, `CURL_IMPERSONATE_PROFILE`), verify the new release ships the wrapper named in `CURL_IMPERSONATE_PROFILE` (lexiforest regenerates wrappers on Chrome major bumps and occasionally drops older ones — pick a profile from the new release's manifest), then run `typeclaw start --build` in any agent folder. The Dockerfile is regenerated on every `start` per the existing "owns the Dockerfile" rule, so the new layer ships automatically. The build itself smoke-tests the binary via `curl_${PROFILE} --version` so a missing or broken profile fails the build, not the first websearch call.
