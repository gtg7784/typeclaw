...
8. **Channel router (`channel.respond` gate, opt-in)**: when `typeclaw.json#permissions.gateChannelRespond` is `true`, every inbound the router sees is additionally gated by `permissions.has(partialOrigin, 'channel.respond')` BEFORE `ensureLive`. The partial origin is built from `(adapter, workspace, chat, thread, lastInboundAuthorId)` — there is no live session yet, so no `participants` / `membership` are passed. A denial logs `[channels] <key>: denied by permissions (channel.respond) author=<id> id=<msg>` and returns without creating a session, updating participants, or warming the membership cache. The flag is read live via a closure in `src/run/index.ts` (`() => getConfig().permissions.gateChannelRespond`), classified `applied` in FIELD_EFFECTS — flipping it via `typeclaw reload` takes effect on the next inbound. Default is `false`: existing configs keep their `channels.<adapter>.allow[]`-only behavior. The router options accept `permissions` and `gateChannelRespond` as a pair — omitting either (or both) reverts to `allow[]`-only. The production wiring in `src/run/index.ts` always passes both; tests that don't exercise the gate omit them.
...
- **Enabling `permissions.gateChannelRespond` without role coverage = silent agent.** When the flag flips on but the user hasn't declared any `roles` block (or the declared roles don't match the speaking authors), every inbound resolves to `guest`, which has no `channel.respond`, which means the router drops every message — `allow[]` rules become irrelevant. The minimum opt-in config is to give a non-`owner` role match rule that covers the conversations the agent should answer (e.g. `"member": { "match": ["slack:T0123/C0GENERAL"] }`). TUI keeps working because `owner.match` always includes `{ kind: 'tui' }` with `channel.respond` granted. The denial log line names the author and the channel key, so a silent agent is diagnosable from logs.
