...
**Entrypoint shim shape.** `buildEntrypointShim()` (`src/init/dockerfile.ts`) emits this sequence when `Xvfb` is on PATH:

1. `Xvfb :99 -screen 0 1920x1080x24 -ac +extension RANDR -nolisten tcp >/dev/null 2>&1 &` — background spawn, no stdout/stderr (the agent uses its own logging).
2. `export DISPLAY=:99` — every subprocess inherits this.
3. Poll-wait up to 500ms for `/tmp/.X11-unix/X99` to exist before exec'ing the agent, so the first child can't race Xvfb startup.
4. Fall through to the network-block check; both `exec bun run typeclaw` (off-path) and `exec setpriv ... -- bun run typeclaw` (on-path) inherit the exported `DISPLAY`.

Xvfb args:

- `:99` — fixed display number. PID-1-isolated namespaces between Compose'd containers keep `/tmp/.X11-unix/X99` collisions impossible across containers.
...
On the network-block path (`TYPECLAW_NETWORK_BLOCK_INTERNAL=1`), the order is `Xvfb spawn → DISPLAY export → iptables rules → setpriv → exec bun`. setpriv drops CAP_NET_ADMIN AFTER Xvfb starts but BEFORE the agent runs — correct, because Xvfb is a userland X server that needs zero capabilities and is already running when setpriv fires. The agent inherits DISPLAY through the exec chain.
...
**Tests covering this contract:** `src/init/dockerfile.test.ts` "wraps both exec paths (off-switch and on-switch) with xvfb-run …" and "drops NET_ADMIN from bounding+inheritable+ambient sets before exec-ing (matches setpriv(1) warning), and wraps the exec target in xvfb-run". `src/container/start.test.ts` "sets --shm-size=2g unconditionally …". Removing either guard without updating the corresponding test will fail CI.
