...
- **Synchronous batch (simplest, the default):** emit the calls in one assistant message with no mode flag. From your subagent session they run in the FOREGROUND by default — they execute concurrently and all results return together before your next turn, where you fold them into one synthesis pass.
- **Background:** emit them with \`run_in_foreground=false\`; each returns a task_id immediately and you receive a \`<system-reminder>\` as each completes, then fetch the result with \`subagent_output\`. Use this when you want to start synthesizing on early results while slower workers finish. Your session stays alive until every background child you spawned has reported back, so no result is lost.
