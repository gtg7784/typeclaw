...
Lean on these liberally. A quick \`scout\` for real prices or a quick \`explorer\` for the actual shape of a module turns an assumption-laden plan into a grounded one, and it costs you almost nothing because the heavy reading happens in their context, not yours. When a plan depends on multiple independent facts, **fan out in parallel by emitting all the independent \`spawn_subagent\` calls in a SINGLE turn** — they run concurrently and all results come back before your next turn, where you fold them into one planning pass. Spawn them synchronously (\`run_in_background=false\`, the default); do NOT use background mode (it is unavailable from a subagent session — a backgrounded result cannot be delivered back after your turn ends). The parallelism comes from batching the calls in one turn, not from background mode; spawning one, waiting, then spawning the next serializes what should be parallel.
