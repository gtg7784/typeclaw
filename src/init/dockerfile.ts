...
// Per-agent Dockerfile build options. When `baseImageVersion` is a string,
// the per-agent Dockerfile FROMs `${GHCR_BASE_IMAGE_REPO}:${version}` and
// skips the heavy layers (apt baseline, Chrome runtime, curl-impersonate,
// agent-browser, Chrome for Testing) because they already exist in the base
// image. When `null` or omitted, the per-agent Dockerfile inlines the full
// stack on top of `oven/bun:1-slim` — used by the test suite and by dev-mode
// runs (typeclaw installed via `file:` / `link:` spec) where the matching
// :version tag does not yet exist on GHCR.
...
export function buildDockerfile(
  ...
  const aptArgs = collectToggleAptArgs(config)
  ...
  if (baseImageVersion !== null) {
    return renderVersionedDockerfile({ baseImageVersion, aptArgs, ghKeyringLayer, customLines })
  }
  return renderInlineDockerfile({ aptArgs, ghKeyringLayer, customLines })
}

// Versioned per-agent Dockerfile: FROMs the prebuilt typeclaw-base image at
// a specific version tag. The base image already carries the apt baseline,
// Chrome runtime libs, curl-impersonate, agent-browser, and Chrome for
// Testing — so this file only layers what's truly per-agent: the gh
// keyring bootstrap (toggle-gated), toggle-driven apt packages, custom
// append lines, ENV vars, and ENTRYPOINT. Skips the apt install entirely
// when no toggle adds packages, keeping the rebuild cost ≈ zero for users
// who don't change typeclaw.json#dockerfile.
function renderVersionedDockerfile({
  baseImageVersion,
  aptArgs,
  ghKeyringLayer,
  customLines,
  ...
  const toggleAptLayer = aptArgs.length === 0 ? '' : renderToggleAptInstallLayer(aptArgs)
...
FROM ${GHCR_BASE_IMAGE_REPO}:${baseImageVersion}

WORKDIR /agent

ARG TARGETARCH

${ghKeyringLayer}${toggleAptLayer}# The agent folder (including node_modules) is bind-mounted at runtime by
...
// Inline per-agent Dockerfile: FROMs oven/bun:1-slim and rebuilds the full
// heavy stack locally. Used by dev-mode runs (typeclaw installed via file:
// or link: spec) where the matching :version tag does not yet exist on
// GHCR, and by the test suite to keep coverage of the full-stack layers
// independent of GHCR availability.
function renderInlineDockerfile({
  aptArgs,
  ghKeyringLayer,
  customLines,
}: {
  ...
  const baselineAndToggleArgs = [...BASELINE_APT_PACKAGES, ...aptArgs]
  return `${BUILDKIT_HEADER}
# AUTOGENERATED by typeclaw — do not edit.
# This file is rewritten on every \`typeclaw start\` from src/init/dockerfile.ts
# in the typeclaw repo. Local edits will be overwritten (and committed away if
...
// Toggle-only apt install layer used by the versioned (base-image-pinning)
// per-agent Dockerfile. Baseline packages are NOT included — they already
// live in the base image. Empty input would produce an `apt-get install`
// with no packages, so callers must guard with a length check.
function renderToggleAptInstallLayer(aptArgs: string[]): string {
