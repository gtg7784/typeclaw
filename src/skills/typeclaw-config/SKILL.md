...
description: Use this skill whenever the user asks you to read or change your runtime configuration, switch model, change the server port, expose a host directory to you, configure which Discord channels you can see or post to, or whenever you yourself need to inspect or edit `typeclaw.json`. Triggers include "what model are you running", "switch to <model>", "change the port", "mount this folder", "give me access to <path>", "let me talk to you in <channel>", "stop replying in <channel>", "add this guild", "edit typeclaw.json", "what's in your config", "use a different provider", or any mention of `typeclaw.json` / runtime settings / host file access / channel allow rules / engagement triggers. Read it before touching `typeclaw.json` — the file has a strict schema, a tiny set of allowed values today, a mix of live-reloadable and restart-required fields, and the channel rules in particular gate what messages you can see at all.
---
...
- **Restart-required fields** (`port`, `mounts`, `plugins`) are reported as "reload landed but change won't apply until restart". The diff returns success; the runtime still has the old value in memory. Tell the user explicitly which one they're hitting.
...
| `mounts`   | no       | array of objects | Host directories bind-mounted into your container. Defaults to `[]` (no host paths exposed). `typeclaw init` writes `[]` explicitly. See **Mounts** section below. **Restart-required.**                                                                                                                            |
...
The runtime fills in defaults for any omitted field: `port` → `8973`, `mounts` → `[]` (no host paths exposed), `plugins` → `[]`, `channels` → `{}` (no adapters configured). New configs scaffolded by `typeclaw init` write `mounts: []` explicitly so the field is visible to the user even when empty. The `memory` block in the scaffold is a **plugin config block** owned by the bundled memory plugin; it is preserved through reloads via `extractPluginConfigs` but its semantics are not this skill's concern — see the `typeclaw-memory` skill.
...
Top-level keys in `typeclaw.json` that are **not** in the well-known six (`$schema`, `port`, `model`, `mounts`, `plugins`, `channels`) are treated as plugin config blocks. The schema preserves them via `catchall(z.unknown())`, and `extractPluginConfigs` hands each block to the owning plugin's `configSchema` for validation at boot.
...
2. **The plugin reads its config once at factory time**, so plugin block edits are effectively `restart-required` even though core's `FIELD_EFFECTS` table doesn't classify them — the well-known six are the only entries `reloadConfig` diffs against.
...
3. **Write the whole file back** with the `write` tool. Always pretty-printed (2-space indent), trailing newline, fields in stable order: `$schema` first, then alphabetical for the rest (`channels`, `model`, `mounts`, `plugins`, `port`, then any plugin config blocks like `memory`).
...
6. **Tell the user the right next step.** Match the field's effect class:
   ...
   - `port`, `mounts`, `plugins` → "Restart-required. Run `typeclaw restart` (host stage) to pick up the change."
...
- No unknown top-level keys you invented — keys outside the well-known six are interpreted as **plugin config blocks** and only do something if a plugin owns them. Inventing one means the user thinks it took effect and it did not.
...
- **Skills directories** (`.agents/skills/`, `skills/`, `memory/skills/`, the bundled `src/skills/`) — these are loaded from disk by the runtime; they are not driven by `typeclaw.json`. See the `typeclaw-skills` skill for the three layers, the `bunx skills` CLI, and the lockfile-based "downloaded vs hand-authored" rule.
