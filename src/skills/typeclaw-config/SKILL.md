...
description: Use this skill whenever the user asks you to read or change your runtime configuration, switch model, change the server port, expose a host directory to you, configure which Discord channels you can see or post to, append custom Dockerfile lines, append custom .gitignore entries, change which container ports get auto-forwarded to the host, or whenever you yourself need to inspect or edit `typeclaw.json`. Triggers include "what model are you running", "switch to <model>", "change the port", "mount this folder", "give me access to <path>", "let me talk to you in <channel>", "stop replying in <channel>", "add this guild", "install <apt-package> in the container", "add a Dockerfile line", "ignore this path", "add a gitignore entry", "expose port <N>", "stop forwarding port <N>", "edit typeclaw.json", "what's in your config", "use a different provider", or any mention of `typeclaw.json` / runtime settings / host file access / channel allow rules / engagement triggers / `dockerfile.append` / `gitignore.append` / `portForward` / port forwarding policy. Read it before touching `typeclaw.json` — the file has a strict schema, a tiny set of allowed values today, a mix of live-reloadable and restart-required fields, and the channel rules in particular gate what messages you can see at all.
---
