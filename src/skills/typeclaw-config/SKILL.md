...
- `thinkingLevel` — reasoning effort applied to new sessions: `off`, `minimal`, `low`, `medium`, `high`, or `xhigh`. Omit to defer to the model SDK's default (`medium`); no provider family is pinned. Clamped to each model's capabilities at session creation (`xhigh` is OpenAI-family-only and clamps down to `high` elsewhere). An attention-escalation cue in a turn still bumps that single turn to maximum effort regardless of this setting. **Live-reloadable.**
...
- **Live-reloadable fields** (`model`, `thinkingLevel`, `alias`, `channels`) take effect on the next `reload` — no container restart.
...
| `thinkingLevel` | no       | string           | Reasoning effort for new sessions: `off` \| `minimal` \| `low` \| `medium` \| `high` \| `xhigh`. Omit to defer to the SDK default (`medium`). Clamped per model at session creation (`xhigh` is OpenAI-family-only). Set it with `typeclaw model set … --thinking <level>`. **Live-reloadable.**                                                                                                                                                                                                                                                                                                                         |
