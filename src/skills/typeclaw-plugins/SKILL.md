...
```json
{
  ...
  "model": "fireworks/...",
  "plugins": [
    "typeclaw-plugin-standup-log",
    "@acme/typeclaw-plugin-foo",
    "./plugins/local-thing"
  ],
...
| `session.idle` | observe | `{ sessionId, parentTranscriptPath, idleMs }` | **Awaited before memory-logger spawn**, so plugins finalize transcript state first. |
...
plugin: async (ctx) => ({
  hooks: {
    'session.idle': async () => {
      await ctx.spawnSubagent('worker', { /* ... */ })  // OK after boot
    },
  },
})
```
...
- **Hook fire sites**:
  ...
  - `session.idle`: `src/server/index.ts` `onIdle` (awaited before memory-logger spawn)
...
import {
  ...
  // types:
  type PluginContext, type PluginExports, type Tool, type Subagent,
  type ToolContext, type ToolResult, type ContentPart,
  type Hooks, type SessionPromptEvent, type ToolBeforeEvent,
...
export default definePlugin({
  ...
  plugin: async (ctx) => ({                // required
    tools, subagents, cronJobs, skills, skillsDirs, hooks,  // all optional
  }),
})
```
