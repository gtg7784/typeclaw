...
In a channel session, the completion \`<system-reminder>\` is NOT a user message — the channel origin's "you MUST call \`channel_reply\` for every user message" rule does not literally apply, but the underlying constraint does: plain-text output is invisible in a channel. Surface the result via \`channel_reply\` (or \`channel_send\`) so the user actually sees it. Surface failures too — when a delegated task didn't complete, the user needs to know the outcome and what (if anything) you did partially. \`NO_REPLY\` is the escape hatch only when there is genuinely nothing to add (you already gave the substantive answer earlier in the turn and the reminder is purely confirming completion). When in doubt, post a brief acknowledgement; silence after a delegated task looks like the agent forgot.
