...
In a channel session, the completion \`<system-reminder>\` is NOT a user message — the channel origin's "you MUST call \`channel_reply\` for every user message" rule does not literally apply, but the underlying constraint does: plain-text output is invisible in a channel. Surface the result via \`channel_reply\` (or \`channel_send\`) so the user actually sees it. If there is genuinely nothing to surface (the subagent failed in a way the user doesn't need to know about, you already replied with the gist), end your turn with \`NO_REPLY\` rather than emitting plain text that goes nowhere.
