...
const dockerfileObjectSchema = z.object({
  ...
  // Install xvfb so the entrypoint shim can wrap the agent in `xvfb-run`,
  // giving headed Chrome (agent-browser --headed, Playwright headful) a
  // virtual X display. Default `true` because modern bot detection
  // (Akamai/Cloudflare Bot Manager) fingerprints `--headless` and
  // `--headless=new` regardless of UA spoof, and headed-via-Xvfb is the
  // cheapest path to a passing fingerprint from a container. Opt-out with
  // `xvfb: false` to save ~5MB image + ~10MB RAM/idle on agents that never
  // touch a browser. The shim self-heals — when xvfb-run isn't on PATH it
  // execs the agent directly, no other Dockerfile or shim change needed.
  // Boolean-only because the package has no API-stable versioning that
  // matters here; xvfb tracks the upstream X server release.
