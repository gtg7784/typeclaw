...
export function createDiscordUserEditMessageCallback(deps: {
  client: Pick<DiscordClient, 'editMessage'>
}): EditMessageCallback {
  return async (req): Promise<EditMessageResult> => {
    if (req.adapter !== 'discord') {
...
// Discord only lets you edit your OWN messages: a PATCH on someone else's
...
// The user client throws DiscordError with the Discord API error code, or
// `http_<status>` when the body carries no code (same shape as discord-bot).
