...
The bash masks alone would be hollow: `read`, `grep`, `find`, `ls`, `edit`, and `write` run in the main process, outside any sandbox, so a guest could read back through them exactly what bash masking denies. The `privateSurfaceRead` guard (`src/bundled-plugins/security/policies/private-surface-read.ts`) closes that — it blocks those tools from the same hidden **directories** at the `tool.before` boundary, driven by the same `resolveHiddenPaths` deny-list. The secret **files** (`.env`, `secrets.json`) are owned by the pre-existing `secretExfilRead` guard, whose medium-severity bypass already maps to the `fs.see.secrets` tier; `privateSurfaceRead` scopes to directories only to avoid double-gating that guard's acknowledgement path.
