...
- **`<agentDir>/secrets.json`** — `v2` envelope managed by `SecretsBackend` (`src/secrets/storage.ts`), wrapping `pi-coding-agent`'s `AuthStorage`. Gitignored.
  ...
  - `channels.<adapter>` — per-adapter fields. `discord-bot`/`telegram-bot`: `{ token }`. `slack-bot`: `{ botToken, appToken }`. `kakaotalk`: `{ currentAccount, accounts: Record<id, KakaoAccountRecord> }` (multi-account, holds `email` + `encryptedPassword` for renewal — see [Host daemon](/docs/internals/hostd)).
...
Writes are `v2`. Three legacy shapes accepted on read and upgraded transparently: v1 envelope, pre-envelope flat shape, pre-rename `auth.json`. Next write produces v2.
...
- **Adding an adapter/provider with credentials** requires three coordinated edits in `src/secrets/defaults.ts` + `schema.ts`: per-adapter field schema, `CHANNEL_FIELD_ENV` entry, and (if v1 upgrade matters) `CHANNEL_ENV_TO_FIELD`. Forgetting the env entry silently breaks injection at boot.
