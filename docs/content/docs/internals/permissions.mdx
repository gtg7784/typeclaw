...
8. **Channel router (`channel.respond` gate)**: every inbound is gated BEFORE `ensureLive`. Denial logs `[channels] <key>: denied by permissions (channel.respond) author=<id> id=<msg>` and returns. Legacy `channels.<adapter>.allow[]` is gone — auto-translated to `roles.member.match[]`.
