...
- **An MCP server** — a stdio or HTTP server whose tools join the agent's MCP catalog
...
<Accordion title="Contributing an MCP server">

Plugins can register MCP servers in their returned `mcpServers` map. The plugin must declare the public `mcp` permission; server names are global, unprefixed, lowercase, and cannot contain `__`.
...
At boot, TypeClaw merges `typeclaw.json#mcpServers` first and plugin servers second. If a plugin name collides with user config, the config server wins and the plugin server is skipped with a warning. A server that later fails to spawn or connect does not crash boot.
