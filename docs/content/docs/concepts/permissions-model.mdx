...
- **`high` — audience-leak.** Bypass sends data to a third-party audience outside the operator's control loop. Examples: outbound channel message with secrets, `git push` to an unfamiliar remote, role/cron promotion. **`owner` bypasses.** `trusted`, `member`, `guest` do not.
- **`medium` — silent-attack.** Bypass produces attacker-favorable state in model context without immediate operator visibility. Examples: `bash env`, `read .env`, IMDS SSRF, `session_search` returning secret-shaped hits. **`owner` and `trusted` bypass.** `member`, `guest` do not.
