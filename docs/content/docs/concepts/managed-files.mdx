...
`typeclaw.json`, `cron.json`, `.env`, `secrets.json`, anything under `workspace/`, anything you create yourself. TypeClaw reads these; it doesn't rewrite them unsolicited. The exceptions are auto-migrations (legacy `dockerfile` → `docker.file`, legacy `allow[]` → `roles.member.match[]`) which run on first load after a schema change and commit the rewrite with a descriptive subject — and even those preserve every byte that doesn't need to change.
