...
Transport: **LOCO protocol** (no public URL required). Setup walkthrough: [Add a channel](/docs/guides/first-channel).
...
Sub-device login using email and password. Credentials are stored **encrypted at rest** (AES-256-GCM; the key lives outside the agent folder).

Stored in [`secrets.json#channels.kakaotalk`](/docs/reference/secrets-json) as an accounts map with these fields:
