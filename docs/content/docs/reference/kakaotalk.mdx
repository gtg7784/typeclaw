...
| Reactions               | no        |                                                                |
| Typing indicator        | yes       | Animated "…" composing dots via the sub-device LOCO session    |
...
- **Typing indicator**: emits the LOCO `ACTION` packet (`type=1`) from the sub-device session so the recipient sees the animated "…" composing dots. KakaoTalk auto-expires the indicator ~5s after the last packet, so the adapter self-refreshes every 4s between the router's 8s heartbeats to keep it continuously visible; a delivered message clears it client-side.
