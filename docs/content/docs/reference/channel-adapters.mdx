...
| `github`       | adapter restarts on tunnel URL change and re-registers the webhook automatically. In **GitHub App** mode an opened PR is treated as a review request (an App can't be added to `requested_reviewers`, so `pull_request.opened` is the only signal it gets); in **PAT** mode the bot is a real user that _can_ be requested, so it waits for an explicit `review_requested` event instead of reviewing every opened PR. Team-reviewer requests are honored when the bot is a member of the requested team. |
