...
| Cross-channel reads      | full      | One of only two adapters (with `slack-bot`) that back all three `channel_read` modes (`history`, `message`, `list`) — see [Cross-channel reads](/docs/reference/channel-adapters#cross-channel-reads) |
